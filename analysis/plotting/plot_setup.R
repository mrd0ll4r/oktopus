...
old_ggplot2_theme <- theme_set(theme_bw(10))
theme_update(
...
print_plot <- function(plot, name, width=3.2, height=2.5){
  tex_name <- sprintf("fig/%s.tex",name)
  png_name <- sprintf("fig/%s.png",name)
  tex_width <- width
  tex_height <- height
  png_width <- tex_width*2
  png_height <- tex_height*2
  
  tikz(file=tex_name, width=tex_width, height=tex_height,sanitize = TRUE)
  print(plot)
  dev.off()
  
  png(file=png_name, width=png_width, height=png_height, units="cm", res=300)
  print(plot)
  dev.off()
}
